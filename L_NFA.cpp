...
string NFA(int S, vector<int> stari, vector <vector<Tranzitie>> lista_adiacenta, vector <int> stari_fin,string cuv){
        ...
        while (stiva.empty() != true){
            ...
            for (int k = 0; k < lista_adiacenta[l].size(); k++){
                // daca litera curenta se afla in lista de vecini ai starii sau ma duc cu lambda
                if (lista_adiacenta[l][k].litera == cuv[x.indice] || lista_adiacenta[l][k].litera == 'l'){
                    ...
                    if (x.indice == cuv.size()-1){
                        ...
                        for (int j = 0; j < lista_adiacenta[l].size(); j++){
                            ...
                            if (lista_adiacenta[l][j].litera == cuv[x.indice]){
                                ...
                                // daca litera vecinului != litera din cuvant care urmeaza 
                                // verific daca litera vecinului este lambda
                                // daca da, atunci avansez si adaug in stiva indicele 
                                // care ramane identic pentru ca nu avansez in cuvant 
                                if (lista_adiacenta[l][j].litera == 'l'){
                                    //cout << lista_adiacenta[l][j].litera <<" "<<lista_adiacenta[l][j].destinatie;
                                    e.indice = x.indice;
                                    stiva.push(e);
                                }
                            }
